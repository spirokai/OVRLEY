...
//! through the public `format_metric_parts` API, including unit conversion,
//! placeholder fallback, and left/right balance format variants.
//!
//! ## Type
...
fn config_json(values_json: &str) -> RenderConfig {
    let input = format!(
        r#"{{
    ...
    ovrley_core::config::parse_config_json(&input).unwrap()
}

fn value_config_json(value_type: &str, overrides: &[(&str, &str)]) -> String {
    let mut fields = format!(r#""value": "{}", "x": 100, "y": 100"#, value_type);
    for (key, val) in overrides {
        fields.push_str(&format!(r#", "{}": {}"#, key, val));
    }
    format!("{{{}}}", fields)
}
...
fn format_parts(kind: &str, series_key: &str, raw: Option<f64>, overrides: &[(&str, &str)]) -> (String, Option<String>) {
    let vc_json = value_config_json(kind, overrides);
    let config = config_json(&format!("[{}]", vc_json));
    let vc = &config.values[0];
    let report = activity_for(series_key, raw);
    let parts = format_metric_parts(&config, vc, &report, 0);
    match parts {
...
fn pace_formats_as_min_km() {
    let (value, unit) = format_parts("pace", "pace", Some(275.0), &[("display_unit", r#""min_per_km""#)]);
...
fn pace_formats_as_min_mi() {
    let (value, unit) = format_parts("pace", "pace", Some(275.0), &[("display_unit", r#""min_per_mi""#)]);
...
fn g_force_formats_as_g() {
    let (value, unit) = format_parts("g_force", "g_force", Some(1.5), &[("display_unit", r#""g""#), ("decimals", "1")]);
    assert_eq!(value, "1.5");
...
fn g_force_converts_to_mps2() {
    let (value, unit) = format_parts("g_force", "g_force", Some(1.0), &[("display_unit", r#""mps2""#), ("decimals", "1")]);
    assert_eq!(value, "9.8");
...
fn air_pressure_formats_as_hpa() {
    let (value, unit) = format_parts("air_pressure", "air_pressure", Some(1.0), &[("display_unit", r#""hpa""#)]);
...
fn air_pressure_converts_to_inhg() {
    let (value, unit) = format_parts("air_pressure", "air_pressure", Some(1.0), &[("display_unit", r#""inhg""#)]);
    assert_eq!(value, "29");
...
fn ground_contact_time_formats_as_ms() {
    let (value, unit) = format_parts("ground_contact_time", "ground_contact_time", Some(250.0), &[("display_unit", r#""ms""#)]);
    assert_eq!(value, "250");
...
fn stride_length_formats_as_m() {
    let (value, unit) = format_parts("stride_length", "stride_length", Some(1.25), &[("display_unit", r#""m""#), ("decimals", "2")]);
...
fn stroke_rate_formats_as_spm() {
    let (value, unit) = format_parts("stroke_rate", "stroke_rate", Some(85.0), &[("display_unit", r#""spm""#)]);
    assert_eq!(value, "85");
...
fn torque_formats_as_nm() {
    let (value, unit) = format_parts("torque", "torque", Some(35.5), &[("display_unit", r#""nm""#), ("decimals", "1")]);
...
fn vertical_speed_formats_as_mps() {
    let (value, unit) = format_parts("vertical_speed", "vertical_speed", Some(5.2), &[("display_unit", r#""mps""#), ("decimals", "1")]);
    assert_eq!(value, "5.2");
...
fn balance_formats_plain_default() {
    let (value, _) = format_parts("left_right_balance", "left_right_balance", Some(52.0), &[("display_unit", r#""percent""#)]);
    assert_eq!(value, "52 / 48");
}

#[test]
fn balance_formats_percent_label() {
    let (value, _) = format_parts("left_right_balance", "left_right_balance", Some(52.0), &[("display_unit", r#""percent""#), ("balance_format", r#""percent_label""#)]);
    assert_eq!(value, "52% / 48%");
}

#[test]
fn balance_formats_plain() {
    let (value, _) = format_parts("left_right_balance", "left_right_balance", Some(60.0), &[("display_unit", r#""percent""#), ("balance_format", r#""plain""#)]);
    assert_eq!(value, "60 / 40");
}

#[test]
fn balance_formats_l_prefix() {
    let (value, _) = format_parts("left_right_balance", "left_right_balance", Some(48.0), &[("display_unit", r#""percent""#), ("balance_format", r#""l_prefix""#)]);
    assert_eq!(value, "L48 / R52");
}

#[test]
fn balance_formats_l_suffix() {
    let (value, _) = format_parts("left_right_balance", "left_right_balance", Some(70.0), &[("display_unit", r#""percent""#), ("balance_format", r#""l_suffix""#)]);
    assert_eq!(value, "70L / 30R");
}

#[test]
fn balance_placeholder_shows_dashes() {
    let (value, _) = format_parts("left_right_balance", "left_right_balance", None, &[("display_unit", r#""percent""#)]);
    assert_eq!(value, "--");
}
