...
D. add the rendering options (fps, widget update rate, export range, codec) also to the render modal -> after pressing the render button, the modal opens, user confirms the render settings and only then the rendering starts. The codec and custom range do not affect the preview, because of that they should be removed from the sidebar, only kept in the modal. FPS and widget update rate does affect both, so they should remain in the sidebar as well as in the modal.
...
[MORE COMPLICATED]
H. after changing frontend preview text in widgets to svg, the shadows are not applied in the preview
