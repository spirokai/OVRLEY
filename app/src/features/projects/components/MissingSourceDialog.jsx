...
export function MissingSourceDialog({ sourceRole, open, onLocate, onLoadAnyway, onCancel }) {
  ...
  return (
    <Dialog open={open}>
      ...
      >
        <DialogTitle className="text-lg font-bold">{sourceRole === 'video' ? 'Video' : 'Activity'} {t('projects.sourceNotFound', 'source not found')}</DialogTitle>
        <DialogDescription className="mt-2 text-sm text-muted-foreground">
          {t('projects.missingSourceDescription', 'The {{sourceRole}} file does not seem to exist. Locate the moved file or cancel without changing the current session.', { sourceRole })}
        </DialogDescription>
