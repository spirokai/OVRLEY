...
└── ovrley_core/              # Core library crate
    ...
    └── src/
        ...
        │   └── mod.rs
        ├── encode/           # Video encoding, ffmpeg, pipelines (~3500 lines)
        │   ├── mod.rs
...
**Target:** Prefer fixtures co-located with tests:
...
│   │   ├── ffmpeg_settings.rs# NEW: ffmpeg command/settings derivation
│   │   ├── ffmpeg_composite.rs
│   │   ├── fps.rs
│   │   ├── progress.rs       # NEW: RenderController moved here
│   │   ├── video.rs
│   │   ├── video_pipeline.rs
...
**6. Move `AppPaths` to neutral module**
...
**2. Extract progress/cancellation ownership into `progress.rs`**

Move `RenderController` and `RenderProgress` from `encode/video.rs` into `encode/progress.rs`.
...
## 17.10 `ovrley_core/src/encode/` (~3500 lines)

**Status:** Largest complexity hotspot.
**Issues:**
- `video.rs` — `RenderController` should be in `progress.rs`
...
**Action:** Extract `ffmpeg_settings.rs`, `progress.rs`. Enforce sibling pipeline isolation. Preserve ffmpeg behavior exactly.
