...
- All gauge track styling fields on `ValueConfig`: `height`, `width`, `track_corner_radius`, `track_border_thickness`, `track_border_color`, `track_empty_color`, `track_empty_opacity`, `track_filled_color`, `track_filled_opacity`, `show_min_max_labels`, `min_max_label_font_size`, `min_max_label_color`.
...
- [x] All shared gauge styling fields added to `ValueConfig` with `#[serde(default)]`
- [x] `GaugeWidgetCache` struct created with static `SkiaImage` and per-frame state, following `RouteWidgetCache`/`ElevationWidgetCache` pattern
- [x] Linear gauge static layer (empty track + border + min/max labels) baked into cached `SkiaImage` during `prepare_render_assets`
- [x] Linear gauge dynamic fill rendered per-frame in `render_frame_rgba`
- [x] Horizontal orientation: fill sweeps left-to-right
- [x] Vertical orientation: fill sweeps bottom-to-top
- [x] Min/max labels rendered correctly when enabled, hidden when disabled
- [x] Track corner radius applied to track rectangle corners
- [x] Border thickness and color rendered around track
- [x] Empty and filled track colors with opacity rendered correctly
- [x] `display_type: "text"` continues to render as before (no regression)
- [x] Frontend `gaugeGeometry.js` module with pure fill percentage and layout functions
- [x] Frontend SVG preview renders linear gauge identically to Skia backend
- [x] Frontend editor controls for all linear gauge options
- [x] Min/max derived from activity data; placeholder 0–100 range shown when no activity loaded
- [x] Rust unit tests for fill percentage calculation, bar sizing formula, and config deserialization
- [x] Frontend tests for `gaugeGeometry` calculations and editor control dispatch
