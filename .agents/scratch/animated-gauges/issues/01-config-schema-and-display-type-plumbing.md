...
Add `display_type` field to the `ValueConfig` struct in the Rust backend config schema. This field controls how a metric value widget is rendered: `"text"` (current behavior), `"linear"`, `"bars"`, `"arc"`, or `"corner"`. These will be default options for metric widgets, however we must allow overrides, e.g. heading widget will only allow "text" and "tape", whereas date/time widget is only "text". Ensure we don't need to define every widget separately, only the "exceptions".
...
- [ ] A `DisplayType` enum is created with variants `Text`, `Linear`, `Bars`, `Arc`, `Corner`, `Tape` and derives `Clone`, `Debug`, `Deserialize`, `Serialize`
...
- [ ] Unknown `display_type` values are handled gracefully (defaulted to `Text`)
